...
bool CvDLLWidgetData::executeAction(CvWidgetDataStruct &widgetDataStruct) {
  bool bHandled = false; //	Right now general bHandled = false;  We can specific case this to true later.  Game will run
                         // with this = false;
      ...
      if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_HEAL) {
        ...
        while (pSelectedUnitNode != NULL) {
          ...
          if (pSelectedUnit->canDiscover(pMissionPlot)) {
            ...
            if (pSelectedUnit->getDiscoverResearch(eTech) >=
                GET_TEAM(pSelectedUnit->getTeam()).getResearchLeft(eTech)) {
          ...
          if (!(pMissionCity->isProductionBuilding())) {
            ...
            while (pSelectedUnitNode != NULL) {
              ...
              if (pSelectedUnit->canHurry(pMissionPlot, true)) {
                ...
                if (NULL != pcKey &&
                    pSelectedUnit->getHurryProduction(pMissionPlot) >= pMissionCity->productionLeft()) {
                  szBuffer.append(NEWLINE);
...
void CvDLLWidgetData::parseConvertHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer) {
  if (widgetDataStruct.m_iData2 != 0) {
    if (widgetDataStruct.m_iData1 == NO_RELIGION) {
      ...
      //			szBuffer.Format(L"Convert to %s", GC.getReligionInfo((ReligionTypes)
      // widgetDataStruct.m_iData1).getDescription());
...
void CvDLLWidgetData::parseFlagHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer) {
  ...
  szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"),
                      GC.getCivilizationInfo(GC.getGameINLINE().getActiveCivilizationType()).getDescription());
  szBuffer.append(szTempBuffer);
  szBuffer.append(NEWLINE);
  ...
  if (pHeadSelectedCity != NULL) {
    if (pHeadSelectedCity->isWeLoveTheKingDay()) {
      ...
      if (iMaintenanceValue != 0) {
        //			swprintf(szTempBuffer, "\n%s%d%c: %s", ((iMaintenanceValue > 0) ?  "+" : ""), iMaintenanceValue,
        // GC.getCommerceInfo(COMMERCE_GOLD).getChar(),
        //((GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).getNumGovernmentCenters() > 0) ? "Distance from Palace" :
        //"No Palace Penalty"));
      ...
      if (iMaintenanceValue != 0) {
        //			swprintf(szTempBuffer, "\n%s%d%c: Number of Cities", ((iMaintenanceValue > 0) ? "+" : ""),
        // iMaintenanceValue, GC.getCommerceInfo(COMMERCE_GOLD).getChar());
        CvWString szMaint = CvWString::format(L"%d.%02d", iMaintenanceValue / 100, iMaintenanceValue % 100);
        szBuffer.append(NEWLINE);
      ...
      //		swprintf(szTempBuffer, "\n%d%c Total Maintenance", pHeadSelectedCity->getMaintenance(),
      // GC.getCommerceInfo(COMMERCE_GOLD).getChar());
...
void CvDLLWidgetData::parseTechPrereqHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer) {
  //	szBuffer.Format(L"%cThis technology requires the knowledge of %s", gDLL->getSymbolID(BULLET_CHAR),
  // GC.getTechInfo((TechTypes) widgetDataStruct.m_iData1).getDescription());
