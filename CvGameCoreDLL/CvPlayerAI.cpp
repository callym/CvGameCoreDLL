...
void CvPlayerAI::AI_init() {
  ...
  if ((GC.getInitCore().getSlotStatus(getID()) == SS_TAKEN) ||
    ...
    // AI_setCivicTimer(((getMaxAnarchyTurns() == 0) ? (GC.getDefineINT("MIN_REVOLUTION_TURNS") * 2) :
    // CIVIC_CHANGE_DELAY) / 2);
                        ...
                        if (canTradeItem(((PlayerTypes)iI), item, true) &&
                          ...
                          if (iTheirValue > iOurValue) {
                            ...
                            for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop)) {
                              ...
                              if (canTradeItem(((PlayerTypes)iI), item, true)) {
                                if (GET_PLAYER((PlayerTypes)iI).AI_cityTradeVal(pLoopCity) <=
  ...
  if (pNearestCity != NULL) {
    if (isBarbarian()) {
      ...
      if (pNearestCity->isCapital()) {
        ...
        iValue *= 100 + (((bAdvancedStart ? 80 : 50) * std::max(0, (iMaxDistanceFromCapital - iDistance))) /
                         iMaxDistanceFromCapital);
        iValue /= 100;
      }
    }
...
bool CvPlayerAI::AI_isAreaAlone(CvArea *pArea) const {
  return ((pArea->getNumCities() - GET_TEAM(BARBARIAN_TEAM).countNumCitiesByArea(pArea)) ==
          GET_TEAM(getTeam()).countNumCitiesByArea(pArea));
}
          ...
          if (!(kTeam.isHasTech((TechTypes)iI))) {
            if (GC.getTechInfo((TechTypes)iI).getEra() <= (getCurrentEra() + 1)) {
              ...
              if (iPathLength <= iMaxPathLength) {
                ...
                int iRandomFactor = ((bAsync) ? GC.getASyncRand().get(2000, "AI Research ASYNC")
                                              : GC.getGameINLINE().getSorenRandNum(2000, "AI Research"));
                int iRandomMax = 2000;
                  ...
                  if (eLoopBuilding != NO_BUILDING) {
                    ...
                    if (isTechRequiredForBuilding(((TechTypes)iI), eLoopBuilding)) {
                      ...
                      if (kLoopBuilding.getMaintenanceModifier() < 0) {
                        ...
                        CvCity *pLoopCity;
                        iTempValue = 0;
                        for (pLoopCity = firstCity(&iLoop, true); pLoopCity != NULL;
                             pLoopCity = nextCity(&iLoop, true)) {
...
int CvPlayerAI::AI_getCloseBordersAttitude(PlayerTypes ePlayer) const {
  if (m_aiCloseBordersAttitudeCache[ePlayer] == MAX_INT) {
    ...
    m_aiCloseBordersAttitudeCache[ePlayer] =
        ((GC.getLeaderHeadInfo(getPersonalityType()).getCloseBordersAttitudeChange() * iPercent) / 100);
  }
      ...
      if (GC.getVoteInfo(eVote).isNoNukes()) {
        ...
        if (AI_isDoStrategy(AI_STRATEGY_OWABWNW)) {
          bValid = false;
                   ...
                   (GC.getGameINLINE().countTotalNukeUnits() /
                    std::max(1, GC.getGameINLINE().countCivPlayersAlive()))) {
          bValid = false;
        }
...
bool CvPlayerAI::AI_counterPropose(PlayerTypes ePlayer, const CLinkList<TradeData> *pTheirList,
  ...
  if (iAIDealWeight > iHumanDealWeight) {
    ...
    if (atWar(getTeam(), GET_PLAYER(ePlayer).getTeam())) {
      ...
      for (pNode = pOurInventory->head(); pNode && iHumanDealWeight > iAIDealWeight;
           pNode = pOurInventory->next(pNode)) {
        if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden) {
          if (pNode->m_data.m_eItemType == TRADE_CITIES) {
      ...
      for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i) {
        if (eBonus == kCorp.getPrereqBonus(i)) {
          ...
          /*if (NO_BONUS != kCorp.getBonusProduced())
          {
              ...
              {
                  iBonusValue += (1000 * iCorpCount * AI_baseBonusVal((BonusTypes)kCorp.getBonusProduced())) / (10 *
          iCityCount);
          }
          }*/
        }
...
int CvPlayerAI::AI_cityTradeVal(CvCity *pCity) const {
  ...
  iValue += (((((pCity->getPopulation() * 50) + GC.getGameINLINE().getElapsedGameTurns() + 100) * 4) *
  ...
  if (!(GET_PLAYER(ePlayer).isHuman())) {
    if (GET_PLAYER(ePlayer).getTeam() != getTeam()) {
      if ((pCity->plot()->calculateCulturePercent(ePlayer) == 0) && !(pCity->isEverOwned(ePlayer)) &&
        ...
        if ((pNearestCity == NULL) || (plotDistance(pCity->getX_INLINE(), pCity->getY_INLINE(),
...
int CvPlayerAI::AI_religionTradeVal(ReligionTypes eReligion, PlayerTypes ePlayer) const {
  ...
  if (eBestReligion != NO_RELIGION) {
    if (eBestReligion != eReligion) {
      iValue += std::max(
          0, (GET_PLAYER(ePlayer).AI_religionValue(eBestReligion) - GET_PLAYER(ePlayer).AI_religionValue(eReligion)));
    }
  }
...
DenialTypes CvPlayerAI::AI_religionTrade(ReligionTypes eReligion, PlayerTypes ePlayer) const {
  ...
  if (getStateReligion() != NO_RELIGION) {
    if (getHasReligionCount(eReligion) <
        std::min((getHasReligionCount(getStateReligion()) - 1), (getNumCities() / 2))) {
      return DENIAL_MINORITY_RELIGION;
    }
  }

  if (AI_getAttitude(ePlayer) <=
      GC.getLeaderHeadInfo(getPersonalityType()).getConvertReligionRefuseAttitudeThreshold()) {
    return DENIAL_ATTITUDE;
  }
...
int CvPlayerAI::AI_areaMissionAIs(CvArea *pArea, MissionAITypes eMissionAI,
      ...
      if (!AI_isDoStrategy(AI_STRATEGY_BIG_ESPIONAGE) &&
          (GET_TEAM(getTeam()).AI_getWarPlan(pSpyPlot->getTeam()) != NO_WARPLAN ||
           AI_getAttitudeWeight(pSpyPlot->getOwner()) <
               (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI) ? 50 : 1))) {
        // Destroy Improvement.
...
int CvPlayerAI::AI_espionageVal(PlayerTypes eTargetPlayer, EspionageMissionTypes eMission, CvPlot *pPlot,
  ...
  //	if (GET_TEAM(getTeam()).getEspionagePointsAgainstTeam(eTargetTeam) - iCost <
  // GET_TEAM(GET_PLAYER(eTargetPlayer).getTeam()).getEspionagePointsAgainstTeam(getTeam()))
  //	{
  //		return 0;
  //	}
...
void CvPlayerAI::AI_doCommerce() {
  ...
  if (isCommerceFlexible(COMMERCE_ESPIONAGE) && !bFirstTech) {
    ...
    if (AI_isFinancialTrouble() || AI_isDoStrategy(AI_STRATEGY_CULTURE3)) {
      ...
      while (getCommerceRate(COMMERCE_ESPIONAGE) < iEspionageTargetRate &&
             getCommercePercent(COMMERCE_ESPIONAGE) < 20) {
        changeCommercePercent(COMMERCE_RESEARCH, -GC.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"));
                ...
                if (pLoopDeal->isCancelable(getID())) {
                  if ((GC.getGameINLINE().getGameTurn() - pLoopDeal->getInitialGameTurn()) >=
                    ...
                    if ((pLoopDeal->getFirstPlayer() == getID()) &&
                        (pLoopDeal->getSecondPlayer() == ((PlayerTypes)iI))) {
                      if (GET_PLAYER((PlayerTypes)iI).isHuman()) {
                ...
                for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++) {
                  if (getNumTradeableBonuses((BonusTypes)iJ) > 1) {
                    if ((GET_PLAYER((PlayerTypes)iI).AI_bonusTradeVal(((BonusTypes)iJ), getID(), 1) > 0) &&
                        (GET_PLAYER((PlayerTypes)iI).AI_bonusVal((BonusTypes)iJ, 1) >
                         AI_bonusVal((BonusTypes)iJ, -1))) {
                ...
                if (!(GET_TEAM(getTeam()).isAtWar(GET_PLAYER((PlayerTypes)iI).getTeam()))) {
                  ...
                  if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID())) {
                    if (GET_TEAM(getTeam()).canDeclareWar(GET_PLAYER((PlayerTypes)iI).getTeam()) &&
                        !GET_TEAM(getTeam()).AI_isSneakAttackPreparing(GET_PLAYER((PlayerTypes)iI).getTeam())) {
    ...
    if (iTeam != getTeam()) {
      ...
      if (kTeam.isAlive()) {
        ...
        if (iCountdown > 0) {
          ...
          if (iCountdown < GET_TEAM(getTeam()).getVictoryDelay(eVictory) &&
              kTeam.getLaunchSuccessRate(eVictory) == 100) {
            bLaunch = false;
            break;
          }
        }
...
int CvPlayerAI::AI_getStrategyHash() const {
  ...
  if (AI_getFlavorValue(AI_FLAVOR_PRODUCTION) >= 2) // 0, 2, 5 or 10 in default xml [augustus 5, frederick 10, huayna 2,
                                                    // jc 2, chinese leader 2, qin 5, ramsess 2, roosevelt 5, stalin 2]
  {
...
int CvPlayerAI::AI_countNumAreaHostileUnits(CvArea *pArea, bool bPlayer, bool bTeam, bool bNeutral,
                                            bool bHostile) const {
  PROFILE_FUNC();
  CvPlot *pLoopPlot;
  int iCount;
...
bool CvPlayerAI::AI_advancedStartDoRoute(CvPlot *pFromPlot, CvPlot *pToPlot) {
  ...
  if (gDLL->getFAStarIFace()->GeneratePath(&GC.getStepFinder(), pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(),
    ...
    if (pNode != NULL) {
      if (pNode->m_iData1 >
          (1 + stepDistance(pFromPlot->getX(), pFromPlot->getY(), pToPlot->getX(), pToPlot->getY()))) {
...
void CvPlayerAI::AI_advancedStartRouteTerritory() {
  ...
  //		{
  //			pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
  //			if ((pLoopPlot != NULL) && (pLoopPlot->getOwner() == getID()) && (pLoopPlot->getRouteType() ==
  // NO_ROUTE))
  //			{
  //				aiPlotGroups.clear();
  ...
  //				{
  //					pLoopPlot2 = plotDirection(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(),
  //(DirectionTypes)iJ); 					if ((pLoopPlot2 != NULL) && (pLoopPlot2->getRouteType() != NO_ROUTE))
  //					{
  //						CvPlotGroup* pPlotGroup = pLoopPlot2->getPlotGroup(getID());
  //						if (pPlotGroup != NULL)
  //						{
  //							if (std::find(aiPlotGroups.begin(),aiPlotGroups.end(), pPlotGroup->getID()) ==
  // aiPlotGroups.end())
  //							{
  ...
  //					if (eBestRoute != NO_ROUTE)
  //					{
  //						doAdvancedStartAction(ADVANCEDSTARTACTION_ROUTE, pLoopPlot->getX_INLINE(),
  // pLoopPlot->getY_INLINE(), eBestRoute, true);
  //					}
  //				}
  //			}
  //		}
        ...
        if (pLoopPlot->getBonusType(getTeam()) != NO_BONUS) {
          ...
          for (int iJ = 0; iJ < NUM_CARDINALDIRECTION_TYPES; iJ++) {
            ...
            if ((pLoopPlot2 != NULL) && (getAdvancedStartVisibilityCost(true, pLoopPlot2) > 0)) {
              ...
              if (((iFoodYield >= 2) && !pLoopPlot2->isFreshWater()) || pLoopPlot2->isHills() ||
                  pLoopPlot2->isRiver()) {
...
int CvPlayerAI::AI_getPlotAirbaseValue(CvPlot *pPlot) const {
  ...
  //		if (plotDistance(iMinFriendlyCityPlot->getX_INLINE(), iMinFriendlyCityPlot->getY_INLINE(),
  // iMinOtherCityPlot->getX_INLINE(), iMinOtherCityPlot->getY_INLINE()) < (iMinOtherCityDistance - 1))
  //		{
  //			return 0;
  //		}
  //	}
  ...
  //	{
  //		CvCity* pNearestCity = GC.getMapINLINE().findCity(iMinOtherCityPlot->getX_INLINE(),
  // iMinOtherCityPlot->getY_INLINE(), NO_PLAYER, getTeam(), false); 		if (NULL == pNearestCity)
  //		{
  //			return 0;
  //		}
  //		if (plotDistance(pNearestCity->getX_INLINE(), pNearestCity->getY_INLINE(), iMinOtherCityPlot->getX_INLINE(),
  // iMinOtherCityPlot->getY_INLINE()) < iRange)
  //		{
  //			return 0;
  //		}
  //	}
