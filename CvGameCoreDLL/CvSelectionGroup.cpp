...
bool CvSelectionGroup::canDoInterfaceMode(InterfaceModeTypes eInterfaceMode) {
  ...
  while (pUnitNode != NULL) {
    ...
    case INTERFACEMODE_GO_TO_TYPE:
      if ((getDomainType() != DOMAIN_AIR) && (getDomainType() != DOMAIN_IMMOBILE)) {
        if (pLoopUnit->plot()->plotCount(PUF_isUnitType, pLoopUnit->getUnitType(), -1, pLoopUnit->getOwnerINLINE()) >
            1) {
          return true;
        }
      }
...
void CvSelectionGroup::setTransportUnit(CvUnit *pTransportUnit) {
  ...
  if (pTransportUnit != NULL) {
    ...
    // setTransportUnit removes the unit from the current group (at least currently), so we have to be careful in the
    // loop here so, loop until we do not load one
    bool bLoadedOne;
    do {
      bLoadedOne = false;
...
void CvSelectionGroup::mergeIntoGroup(CvSelectionGroup *pSelectionGroup) {
  ...
  // AI_setUnitAIType removes the unit from the current group (at least currently), so we have to be careful in the loop
  // here so, loop until we have not changed unit AIs
  bool bChangedUnitAI;
  do {
    bChangedUnitAI = false;

    // loop over all the units, moving them to the new group,
    // stopping if we had to change a unit AI, because doing so removes that unit from our group, so we have to start
    // over
    CLLNode<IDInfo> *pUnitNode = headUnitNode();
