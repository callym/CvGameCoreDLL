...
int CvUnitAI::AI_attackOdds(const CvPlot *pPlot, bool bPotentialEnemy) const {
  ...
  iDamageToUs = std::max(1, ((GC.getDefineINT("COMBAT_DAMAGE") * (iTheirFirepower + iStrengthFactor)) /
                             (iOurFirepower + iStrengthFactor)));
  iDamageToThem = std::max(1, ((GC.getDefineINT("COMBAT_DAMAGE") * (iOurFirepower + iStrengthFactor)) /
  ...
  if (getDomainType() != DOMAIN_AIR) {
    iNeededRoundsUs = std::max(
        1, iNeededRoundsUs -
               (iBaseOdds * ((pDefender->immuneToFirstStrikes() ? 0 : (firstStrikes() + chanceFirstStrikes() / 2))) /
                100));
    iNeededRoundsThem = std::max(
        1,
        iNeededRoundsThem -
            ((100 - iBaseOdds) *
             ((immuneToFirstStrikes() ? 0 : (pDefender->firstStrikes() + pDefender->chanceFirstStrikes() / 2))) / 100));
  }
...
void CvUnitAI::AI_workerMove() {
  ...
  //								eImprovement =
  //(ImprovementTypes)GC.getBuildInfo((BuildTypes)pCity->AI_getBestBuild(iI)).getImprovement();
...
void CvUnitAI::AI_attackCityMove() {
  ...
  //			((area()->getAreaAIType(getTeam()) == AREAAI_DEFENSIVE) && (getGroup()->getNumUnits() >=
  // AI_stackOfDoomExtra())))
  if (bReadyToAttack) {
...
void CvUnitAI::AI_counterMove() {
  ...
  if (plot()->getOwnerINLINE() == getOwnerINLINE()) {
    if (AI_load(UNITAI_ASSAULT_SEA, MISSIONAI_LOAD_ASSAULT, UNITAI_ATTACK_CITY, -1, -1, -1, -1, MOVE_SAFE_TERRITORY,
                4)) {
      return;
    }
...
void CvUnitAI::AI_escortSeaMove() {
  ...
  //	// note, this should not happen when we are not the group head, so escort galleons are fine joining a group,
  // just not as head 	if (hasCargo() && (getUnitAICargo(UNITAI_ATTACK_CITY) > 0 || getUnitAICargo(UNITAI_ATTACK) > 0))
  //	{
  //		// non-zero AI_unitValue means that UNITAI_ASSAULT_SEA is valid for this unit (that is the check used
  // everywhere) 		if (GET_PLAYER(getOwnerINLINE()).AI_unitValue(getUnitType(), UNITAI_ASSAULT_SEA, NULL) > 0)
  //		{
...
void CvUnitAI::AI_assaultSeaMove() {
  ...
  // if we are in a city, and at/preparing land war, and we have cargo, unload, even if full, since
  // AI_assaultSeaTransport already had its chance
...
int CvUnitAI::AI_promotionValue(PromotionTypes ePromotion) {
  ...
  for (iI = 0; iI < NUM_DOMAIN_TYPES; iI++) {
    ...
    // iTemp = ((int)((GC.getPromotionInfo(ePromotion).getDomainModifierPercent(iI) +
    // getExtraDomainModifier((DomainTypes)iI)) * 100.0f));
...
bool CvUnitAI::AI_guardCity(bool bLeave, bool bSearch, int iMaxPath) {
  ...
  if (bSearch) {
    ...
    if ((pBestPlot != NULL) && (pBestGuardPlot != NULL)) {
      ...
      // false, false, MISSIONAI_GUARD_CITY, pBestGuardPlot); 			return true;
      ...
      if (canDestroy(pBestCity->plot())) {
        if (pBestCity->getProduction() > ((pBestCity->getProductionNeeded() * 2) / 3)) {
          if (pBestCity->isProductionUnit()) {
            if (isLimitedUnitClass(
                    (UnitClassTypes)(GC.getUnitInfo(pBestCity->getProductionUnit()).getUnitClassType()))) {
              getGroup()->pushMission(MISSION_DESTROY);
              return true;
            }
          } else if (pBestCity->isProductionBuilding()) {
            if (isLimitedWonderClass((BuildingClassTypes)(GC.getBuildingInfo(pBestCity->getProductionBuilding())
  ...
  for (pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL;
    ...
    if (AI_plotValid(pLoopCity->plot())) {
      if (canHurry(pLoopCity->plot())) {
        if (!(pLoopCity->plot()->isVisibleEnemyUnit(this))) {
          if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopCity->plot(), MISSIONAI_HURRY, getGroup()) ==
              0) {
            if (generatePath(pLoopCity->plot(), 0, true, &iPathTurns)) {
              bHurry = false;
...
bool CvUnitAI::AI_bombardCity() {
  ...
  if (canBombard(plot())) {
    ...
    // could also do a compare stacks call here if we wanted, the downside of that is that we may just have a lot more
    // units we may not want to suffer high casualties just to save a turn
    // getGroup()->AI_compareStacks(pBombardCity->plot(), /*bPotentialEnemy*/ true, /*bCheckCanAttack*/ true,
    // /*bCheckCanMove*/ true); int iOurStrength = pBombardCity->plot()->AI_sumStrength(getOwnerINLINE(), NO_PLAYER,
    // DOMAIN_LAND, false, false, false)
                  ...
                  if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_PILLAGE, getGroup()) ==
                    ...
                    if (generatePath(pLoopPlot, 0, true, &iPathTurns)) {
                      ...
                      if (iPathTurns <= iRange) {
                        ...
                        // if not at war with this plot owner, then devalue plot if we already inside this owner's
                        // borders (because declaring war will pop us some unknown distance away)
                        if (!isEnemy(pLoopPlot->getTeam()) && plot()->getTeam() == pLoopPlot->getTeam()) {
                          iValue /= 10;
                        }
...
bool CvUnitAI::AI_found() {
  ...
  //		pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
  //
  //		if (AI_plotValid(pLoopPlot) && (pLoopPlot != plot() ||
  // GET_PLAYER(getOwnerINLINE()).AI_getPlotDanger(pLoopPlot, 1) <= pLoopPlot->plotCount(PUF_canDefend, -1, -1,
  // getOwnerINLINE())))
  //		{
  ...
  //					if (!(pLoopPlot->isVisibleEnemyUnit(this)))
  //					{
  //						if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_FOUND,
  // getGroup(), 3) == 0)
  //						{
  ...
  for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++) {
    ...
    if (pLoopPlot->isCoastalLand()) {
      if (pLoopPlot->getOwnerINLINE() == eBestPlayer) {
        ...
        if (iValue > 0) {
          if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_ATTACK_SPY, getGroup(), 4) ==
              0) {
            if (generatePath(pLoopPlot, 0, true, &iPathTurns)) {
              iValue *= 1000;
                    ...
                    if (bValid) {
                      if (!(pLoopPlot->isVisibleEnemyUnit(this))) {
                        if (GET_PLAYER(getOwnerINLINE())
                          ...
                          if (generatePath(pLoopPlot, 0, true,
                                           &iPathTurns)) // XXX should this actually be at the top of the loop? (with
                                                         // saved paths and all...)
                          {
                            iValue = 10000;
...
bool CvUnitAI::AI_improveBonus(int iMinValue, CvPlot **ppBestPlot, BuildTypes *peBestBuild, int *piBestValue) {
            ...
            if ((eBestTempBuild != NO_BUILD) || (bCanRoute && !bIsConnected)) {
              if (generatePath(pLoopPlot, 0, true, &iPathTurns)) {
                ...
                if (bDoImprove) {
                  ...
                  // iValue += (GC.getImprovementInfo((ImprovementTypes)
                  // GC.getBuildInfo(eBestTempBuild).getImprovement()))
                  iValue +=
  ...
  if (pBestPlot != NULL) {
    if (eBestBuild != NO_BUILD) {
      ...
      /*else
      {
          // the plot may be connected, but not connected to capital, if capital is not on same area, or if civ has no
      capital (like barbarians) FAssertMsg(false, "Expected that a route could be built to eBestPlot");
      }*/
    } else {
      FAssert(false);
    }
...
bool CvUnitAI::AI_defendPlot(CvPlot *pPlot) {
  ...
  if (pCity != NULL) {
    ...
    if (pPlot->plotCount(PUF_canDefendGroupHead, -1, -1, getOwnerINLINE()) <= ((atPlot(pPlot)) ? 1 : 0)) {
      ...
      //			if (pPlot->defenseModifier(getTeam(), false) >= 50 && pPlot->isRoute() && pPlot->getTeam() ==
      // getTeam())
      //			{
      //				return true;
      //			}
